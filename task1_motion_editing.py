...
def interpolation(left_data, right_data, t, method='linear', return_first_key=True):
    ...
    if method == 'linear':
        ...
        joint_kf = np.zeros(shape=(t-1, len(left_data), len(left_data[0])))
        for joint_idx in range(len(left_data)):
            ...
            for i in range(len(joint_kf)):
                joint_kf[i][joint_idx] =  interp_rots[i]
        res = np.concatenate((res, joint_kf), axis=0)
        return res
    ########## Code End ############
...
def concatenate_two_motions(motion1, motion2, last_frame_index, start_frame_indx, between_frames, searching_frames=20, method='interpolation'):
    ...
    sim_matrix = np.zeros(shape=(search_win1.shape[0], search_win2.shape[0]))
    for i in range(search_win1.shape[0]):
        for j in range(search_win2.shape[0]):
            sim_matrix[i][j] = np.linalg.norm(search_win1[i] - search_win2[j])
    ...
    #for position
    search_win1 = motion1.local_joint_positions[last_frame_index - searching_frames:last_frame_index + searching_frames]
    motion2.local_joint_positions = motion2.local_joint_positions + (motion1.local_joint_positions[0] - motion2.local_joint_positions[0])
    search_win2 = motion2.local_joint_positions[max(0, start_frame_indx - searching_frames):start_frame_indx + searching_frames]
